...
def make_xy_plot(grid_return):
    ...
    with h_main:
        ...
        chart=(
            ...
            .encode(**kwds)
            .configure_axis(labelFontSize=16, titleFontSize=16, titleFontWeight='bold')
...
def get_xy_options(ctypes):
    ...
    with st.popover('Settings :material/tune:'):
        # scale properties
        opts['x_scale'] = st.selectbox('X-Axis Scale:', **scale_props['x_scale'])
        opts['y_scale'] = st.selectbox('Y-Axis Scale:', **scale_props['y_scale'])
        # mark properties
        opts['opacity'] = st.slider('Opacity:', **mark_props['opacity'])
        opts['size'] = st.slider('Size:', **mark_props['size'])
        opts['strokeWidth'] = st.slider('Stroke Width:', **mark_props['strokeWidth'])
        opts['color'] = st.color_picker('Color:', **mark_props['color'])
        opts['filled'] = st.checkbox('Fill Markers:', **mark_props['filled'])

    opts['x_axis'] = st.selectbox('X-Axis:', ctypes['num_columns'], index=default_x)
    opts['y_axis'] = st.selectbox('Y-Axis:', ctypes['num_columns'], index=default_y)
    opts['color_by'] = st.selectbox('Color:', ctypes['cat_columns'], index=None)
    opts['size_by'] = st.selectbox('Size:', ctypes['all_columns'], index=None)
    opts['shape_by'] = st.selectbox('Shape:', ctypes['all_columns'], index=None)
    ...
    opts['add_tooltips'] = st.multiselect('Tooltips:', ctypes['all_columns'], default=names_list)
